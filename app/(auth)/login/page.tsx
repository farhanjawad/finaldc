    ...
    <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl p-8 sm:p-10 border border-slate-200 dark:border-slate-800 shadow-xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        <button
          type="submit"
          disabled={isPending}
          className="w-full flex items-center justify-center gap-2 py-3.5 px-6 rounded-xl bg-emerald-600 hover:bg-emerald-700 active:scale-95 disabled:opacity-70 text-white font-bold text-sm shadow-md shadow-emerald-600/20 transition-all mt-2 cursor-pointer"
        >
          {isPending ? (
            <>
...
function LoginLoadingFallback() {
  return (
    <div className="w-full max-w-md bg-white dark:bg-slate-900 rounded-3xl p-10 border border-slate-200 dark:border-slate-800 flex flex-col items-center justify-center shadow-xl">
      <Loader2 className="w-8 h-8 animate-spin text-emerald-600 mb-2" />
      <p className="text-xs text-slate-400 font-medium">লোড হচ্ছে...</p>
    </div>
  );
}
